...
def trans_in_b10(nbr):
    nbr_list = [int(i) for i in nbr if i != ' ']
    while len(nbr_list) <= 32:
        nbr_list.append(0)
    semn = nbr_list[0]
    caract = sum(nbr_list[i]*(2**(8-i)) for i in range(8, 0, -1))
    mantisa = sum(nbr_list[i]*((2**(i-8))**(-1)) for i in range(9, 33))
    valoare = 0
    # valoare normala12
    if 0 < caract < 255:
    ...
    elif caract == 0 and semn == 0:
        valoare = '0'
    elif caract == 0 and semn == 1:
        valoare = '-0'
...
def trans_in_b2(decimal_nbr): #transformarea nbrului in binar
    ...
    final_nbr = ''
    while nbr:
        final_nbr += str(nbr.pop())
    nbr.clear()
    while fractional_nbr != 0:
        fractional_nbr *= 2
        aux = math.floor(fractional_nbr) if fractional_nbr >= 1 else 0
        nbr.append(aux)
        fractional_nbr -= aux
    str(final_nbr)
    new_aux = ''
